login-intro = ncog.link leverages the security and trust of major providers for logging in. Instead of needing to remember another password, simply use one of the providers below to log in or create an account.

storage-agreement = Signing in requires that ncog.link utilize Local Storage which is a technology similar to Cookies. We do not currently have a privacy policy as this product is so early in development.
